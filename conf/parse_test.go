...
func TestParseDigestDoesNotRemoveUsedVariables(t *testing.T) {
	...
	// Ensure that the 'port' key is still present in the returned mapping
	// even though it was referenced as a variable by 'monitor_port'.
	tk, ok := m["port"]
	if !ok {
		t.Fatal("Expected 'port' key to be present in the returned mapping")
	}
	tok := tk.(*token)
	if v, ok := tok.Value().(int64); !ok || v != 4222 {
		t.Fatalf("Expected port value to be 4222, got %v", tok.Value())
	}

	tk, ok = m["monitor_port"]
	if !ok {
		t.Fatal("Expected 'monitor_port' key to be present in the returned mapping")
	}
	tok = tk.(*token)
	if v, ok := tok.Value().(int64); !ok || v != 4222 {
		t.Fatalf("Expected monitor_port value to be 4222, got %v", tok.Value())
	}
}
