...
func TestJetStreamConsumerPullMaxWaiting(t *testing.T) {
	...
	t.Run("parallel pull subscribers", func(t *testing.T) {
		// Ensure no pending pull requests.
		info, err := sub.ConsumerInfo()
		if err != nil {
			t.Fatal(err)
		}
		...
		var (
			...
			errCh       = make(chan error, max)
			expectedMap = make(map[string]bool)
			psubs       = make([]*nats.Subscription, 0)
		)
		...
		for i := 0; i < max; i++ {
			...
			psub, err := js.PullSubscribe(subject, "durable", nats.PullMaxWaiting(5))
			if err != nil {
				t.Error(err)
			}
			...
			go func() {
				...
				// These may receive some messages but they are not going
				// to return until the context cancels the request forcefully later.
				msgs, err := psub.Fetch(1000, nats.Context(ctx))
				if err != nil {
					errCh <- err
				}
				for _, msg := range msgs {
		...
		// Use one of the pull subscribers that were created initially,
		// these are blocked until the context is canceled in the client.
		subB := psubs[0]
