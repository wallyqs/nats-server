...
func TestJetStreamClusterLimitsBasedStreamFileStoreDesync(t *testing.T) {
	...
	// NOTE: Letting things stabilize at the beginning seem to help things sometimes?
	// time.Sleep(15 * time.Second)

	// Confirm the leader at the beginning of the test...
	{
		leaderSrv := c.streamLeader("js", "LTEST")
		t.Logf("Leader at beginning of test is %v", leaderSrv.Name())
	}
...
Setup:
	...
	// unexpectedMsgs := make([]*nats.Msg, 0)
	for subject, entry := range receivedMap {
		found, ok := shouldDrop[subject]
		if ok {
			t.Logf("WRN: Should have dropped message published on %q since got error: %v", subject, found)
			meta, _ := entry.Metadata()
			t.Logf("  ENTRY[%s]: %+v", subject, meta)
			// unexpectedMsgs = append(unexpectedMsgs, entry)
		}
	}
	...
	checkState := func(t *testing.T) error {
		...
		for _, srv := range c.servers {
			...
			// t.Logf("Replica: %v - Stream: %+v", srv.Name(), stream.State)
	...
	t.Logf("Leader::State: %+v", streamLeader.State)
	cinfo, err := psub.ConsumerInfo()
	require_NoError(t, err)
	// t.Logf("Consumer::Delivered: %+v", cinfo.Delivered)
	// t.Logf("Consumer::AckFloor : %+v", cinfo.AckFloor)
	...
	// NOTE: The following section is flaky so it will not fail every time.
	var (
		lastErr  error
		attempts int
	)
...
Consume2:
	...
	if lastErr == nil {
		t.Logf("TEST FINISHED WITH REPLICAS IN SYNC")
		return
	}
