...
func (s *Server) streamSnapshot(acc *Account, mset *stream, sr *SnapshotResult, req *JSApiStreamSnapshotRequest) {
	...
	for index := 1; ; index++ {
		...
		case err := <-eCh:
			...
			// Channel was closed on success, disable this case.
			// The pipe is also closed so io.ReadFull below will
			// return an error and we will break out of the loop.
			eCh = nil
	...
	// Check if the snapshot goroutine reported an error that we may have
	// missed while we were blocked in io.ReadFull.
	select {
	case err := <-sr.errCh:
		if err != _EMPTY_ {
			hdr = []byte(fmt.Sprintf("NATS/1.0 500 %s\r\n\r\n", err))
