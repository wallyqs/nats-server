...
func (fs *fileStore) NumPending(sseq uint64, filter string, lastPerSubject bool) (total, validThrough uint64, err error) {
	...
	matchPsim := func(fn func([]byte, *psi)) {
		fs.psim.Match(stringToBytes(filter), fn)
	}
	matchFss := func(fss *stree.SubjectTree[SimpleState], fn func([]byte, *SimpleState)) {
		fss.Match(stringToBytes(filter), fn)
	}

	return fs.numPendingLocked(sseq, lastPerSubject, isAll, isMatch, matchPsim, matchFss)
}
...
func (fs *fileStore) NumPendingMulti(sseq uint64, sl *gsl.SimpleSublist, lastPerSubject bool) (total, validThrough uint64, err error) {
	...
	matchPsim := func(fn func([]byte, *psi)) {
		stree.IntersectGSL(fs.psim, sl, fn)
	}
	matchFss := func(fss *stree.SubjectTree[SimpleState], fn func([]byte, *SimpleState)) {
		stree.IntersectGSL(fss, sl, fn)
	}

	return fs.numPendingLocked(sseq, lastPerSubject, isAll, isMatch, matchPsim, matchFss)
}

// numPendingLocked is the shared implementation for NumPending and NumPendingMulti.
// The caller is responsible for acquiring fs.mu (at least RLock), determining isAll,
// and providing the matcher closures that abstract over single-filter vs sublist matching.
func (fs *fileStore) numPendingLocked(
	sseq uint64,
	lastPerSubject bool,
	isAll bool,
	isMatch func(string) bool,
	matchPsim func(func([]byte, *psi)),
	matchFss func(*stree.SubjectTree[SimpleState], func([]byte, *SimpleState)),
) (total, validThrough uint64, err error) {
	...
	if lastPerSubject {
		...
		matchPsim(func(subj []byte, psi *psi) {
	...
	// If scanning backward from the beginning would require visiting more than half the blocks, scan forward instead.
	if seqStart >= (len(fs.blks) / 2) {
		for i := seqStart; i < len(fs.blks); i++ {
			...
			var havePartial bool
			var updateLLTS bool
			matchFss(mb.fss, func(bsubj []byte, ss *SimpleState) {
				if havePartial {
					// If we already found a partial then don't do anything else.
					return
				}
