...
func newFileStoreWithCreated(fcfg FileStoreConfig, cfg StreamConfig, created time.Time, prf, oldprf keyGen) (fs *fileStore, err error) {
	...
	fs.mu.Lock()
	// Use defer to ensure the lock is released if any of the enforcement operations panic.
	// This prevents the deadlock that occurred in production when a panic left the lock held.
	unlocked := false
	defer func() {
		if !unlocked {
			fs.mu.Unlock()
