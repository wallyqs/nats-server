...
func TestConfigCheck(t *testing.T) {
	tests := []struct {
		...
		{
			name: "when accounts exports both stream and services type at the same time",
			config: `
                port = 4222
		...
		accounts = {
                ...
                http_port = 8222
				`,
			newDefaultErr: errors.New(`Detected service but already saw a stream: bar`),
			errorLine:     7,
			errorPos:      39,
		},
		{
		...
		accounts = {
                ...
                http_port = 8222
				`,
			newDefaultErr: errors.New(`Detected stream but already saw a service: bar`),
			errorLine:     9,
			errorPos:      25,
		},
		{
		...
		{
			name: "when accounts imports both stream and services type at the same time",
			config: `
		port = 4222
		...
		accounts = {
		  nats {
		    imports = [
		      {
                        stream:  { account: "hello", subject: "world" },
                        service: { account: "hello", subject: "world" }
		      }
		    ]
		  }
		}
		...
		http_port = 8222
				`,
			newDefaultErr: errors.New(`Detected service but already saw a stream`),
			errorLine:     9,
			errorPos:      25,
		},
	}
