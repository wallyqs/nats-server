...
func (c *client) readLoop() {
	...
	// Close over the concrete type to avoid indirection
	// from reading via the interface.
	switch conn := nc.(type) {
	case *tls.Conn:
		read = func() (int, error) {
			return conn.Read(b)
...
func (c *client) processPub(arg []byte) error {
	...
	for ; i < end; i++ {
		...
		if b == ' ' || b == '\t' {
			subject := c.pa.subjectb[:0]
			c.pa.subject = append(subject, arg[start:i]...)
			break
		}
	}
	...
	for j = end - 1; ; j-- {
		if i == j {
			...
			// e.g. PUB hello 5\r\n
			szb := c.pa.szbb[:0]
			c.pa.szb = append(szb, arg[j+1:end+1]...)
		...
		if b == ' ' || b == '\t' {
			...
			if arg[i+1] != ' ' && arg[j-1] != ' ' {
				reply := c.pa.replyb[:0]
				c.pa.reply = append(reply, arg[i+1:j]...)
				return nil
			}
	...
	for k = j - 1; k > i; k-- {
		...
		if b != ' ' && b != '\t' {
			...
			for ; l < k; l++ {
				...
				if b != ' ' && b != '\t' {
					reply := c.pa.replyb[:0]
					c.pa.reply = append(reply, arg[l:k+1]...)
					return nil
				}
			}
			return nil
