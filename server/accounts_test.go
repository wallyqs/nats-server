...
func TestAccountUpdateRemoteServerDisconnectsOldestFirst(t *testing.T) {
	cf := createConfFile(t, []byte(`
          port: -1
          accounts {
            TEST {
	...
	clientsToDisconnect := acc.updateRemoteServer(remoteServerMsg)

	// Should want to disconnect 2 clients (7 total - 5 limit = 2 over).
	require_Equal(t, len(clientsToDisconnect), 2)

	// Verify the returned clients are in chronological order.
	var clientStartTimes []time.Time
	for _, client := range clientsToDisconnect {
		clientStartTimes = append(clientStartTimes, client.start)
	}
	if !clientStartTimes[0].Before(clientStartTimes[1]) {
		t.Fatalf("Expected clients to be in chronological order: %v is after %v", clientStartTimes[0], clientStartTimes[1])
	}

	// The disconnected clients should be the oldest ones we created.
	disconnected := make(map[uint64]bool)
	for _, client := range clientsToDisconnect {
		disconnected[client.cid] = true
	}
	...
	// The first two clients in the sorted list should be the ones selected for disconnection
	require_True(t, disconnected[byStartTime[0].cid])
	require_True(t, disconnected[byStartTime[1].cid])
	require_False(t, disconnected[byStartTime[2].cid])
}
