...
type jetStream struct {
	...
	apiTotal      int64
	apiErrors     int64
	apiTraffic      [JSAPITypeCount]int64              // Per-API-type traffic counters
	apiLatency      [JSAPITypeCount]*jsAPILatencyTracker // Per-API-type latency trackers
	acksTotal       int64
...
func (js *jetStream) apiStats() JSAPITrafficStats {
	...
	for i := JSAPIType(0); i < JSAPITypeCount; i++ {
		var count uint64
		// Special cases for high-traffic counters stored separately
		if i == JSAPIAck {
			count = uint64(atomic.LoadInt64(&js.acksTotal))
		} else if i == JSAPIHeartbeat {
			count = uint64(atomic.LoadInt64(&js.heartbeatsTotal))
		} else {
			count = uint64(atomic.LoadInt64(&js.apiTraffic[i]))
		}
		if count == 0 {
			continue
		}
		opStats := &JSAPIOpStats{Count: count}
		if tracker := js.apiLatency[i]; tracker != nil {
			opStats.P50, opStats.P90, opStats.P99 = tracker.percentiles()
		}
		stats[jsAPITypeNames[i]] = opStats
	}
	return stats
}
