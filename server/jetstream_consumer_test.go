...
// TestJetStreamConsumerInfoAccurateNumPending tests that infoWithAccurateNumPending()
// returns accurate NumPending by forcing a full recalculation via streamNumPending(),
// while info() uses the optimized checkNumPending() path that caps values.
// This ensures JSZ monitoring gets accurate values.
func TestJetStreamConsumerInfoAccurateNumPending(t *testing.T) {
	s := RunBasicJetStreamServer(t)
	defer s.Shutdown()
	...
	// infoWithAccurateNumPending() uses streamNumPending() for accurate count
	ciAccurate = o.infoWithAccurateNumPending()
	require_Equal(t, ciAccurate.NumPending, 10) // Accurate recalculation

	// Verify JSZ also gets accurate NumPending via Jsz() which uses infoWithAccurateNumPending()
