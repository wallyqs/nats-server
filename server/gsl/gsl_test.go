...
//
// BUG: HasInterestStartingIn does not acquire any lock, so calling it
// while another goroutine is modifying the sublist causes a data race.
func TestGenericSublistHasInterestStartingInRace(t *testing.T) {
	...
	// Goroutine 1: repeatedly call HasInterestStartingIn (NO LOCK - BUG!)
	go func() {
	...
	// Goroutine 2: repeatedly modify the sublist (ACQUIRES LOCK)
	go func() {
		for i := 0; i < iterations; i++ {
			val := 1000 + i
	...
	// Goroutine 3: also call HasInterest (which does lock correctly)
	go func() {
		for i := 0; i < iterations; i++ {
