...
func (l *leafNodeOption) Apply(s *Server) {
	...
	for lrc := range s.leafRemoteCfgs {
		...
		if rlo.tlsFirstChanged {
			...
			s.Noticef("Reloaded: LeafNode Remote %s TLS HandshakeFirst value is: %v",
				getLeafNodeRemoteName(lrc.RemoteLeafOpts), rlo.opts.TLSHandshakeFirst)
		}
	...
	for _, r := range enable {
		s.startGoRoutine(func() { s.connectToRemoteLeafNode(r, true) })
	}
...
func checkConfigsEqual(c1, c2 any, ignoreFields []string) error {
	...
	for i := 0; i < oldConfig.NumField(); i++ {
		...
		if !reflect.DeepEqual(oldValue, newValue) {
			return fmt.Errorf("field %q was %v now %v", field.Name, oldValue, newValue)
		}
	}
	return nil
}

func usersHaveChanged(old, new []*User) bool {
	if len(old) != len(new) {
		return true
	}
	if old == nil && new == nil {
	...
	oua := make(map[string]*User, len(old))
	for _, u := range old {
		oua[u.Username] = u
	}
	for _, nu := range new {
		u, ok := oua[nu.Username]
		if !ok {
			return true
		}
...
func getLeafNodeOptionsChanges(s *Server, old, new *LeafNodeOpts) (*leafNodeOption, error) {
	...
	if usersHaveChanged(old.Users, new.Users) {
		return nil, fmt.Errorf("field \"Users\" was %v now %v", old.Users, new.Users)
	}
