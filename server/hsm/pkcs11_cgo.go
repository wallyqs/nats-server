...
func GetSigner(cfg *Config, publicKey crypto.PublicKey) (Signer, error) {
	...
	if err := ctx.Initialize(); err != nil {
		...
		return nil, fmt.Errorf("hsm: C_Initialize failed: %v", err)
	}
	...
	if err != nil {
		cleanup()
		return nil, fmt.Errorf("hsm: C_GetSlotList failed: %v", err)
	}
	if len(slots) == 0 {
		cleanup()
	...
	if err != nil {
		cleanup()
		return nil, fmt.Errorf("hsm: C_OpenSession failed: %v", err)
	}
	...
	if cfg.Pin != "" {
		if err := ctx.Login(session, pkcs11.CKU_USER, cfg.Pin); err != nil {
			sessionCleanup()
			return nil, fmt.Errorf("hsm: C_Login failed: %v", err)
		}
	}
	...
	if err := ctx.FindObjectsInit(session, template); err != nil {
		sessionCleanup()
		return nil, fmt.Errorf("hsm: C_FindObjectsInit failed: %v", err)
	}
	...
	if err != nil {
		sessionCleanup()
		return nil, fmt.Errorf("hsm: key search failed: %v", err)
	}
	if len(objs) == 0 {
		sessionCleanup()
	...
	if err != nil {
		sessionCleanup()
		return nil, fmt.Errorf("hsm: failed to get key type: %v", err)
	}
	if len(attrs) == 0 {
		sessionCleanup()
		return nil, fmt.Errorf("hsm: key type attribute not found")
	}
	...
	case *rsa.PublicKey:
		if keyType != pkcs11.CKK_RSA {
			sessionCleanup()
			return nil, fmt.Errorf("hsm: certificate has RSA key but HSM key type is 0x%x", keyType)
		}
	case *ecdsa.PublicKey:
		if keyType != pkcs11.CKK_EC {
			sessionCleanup()
			return nil, fmt.Errorf("hsm: certificate has EC key but HSM key type is 0x%x", keyType)
		}
	default:
		sessionCleanup()
...
func (s *hsmSigner) Sign(rand io.Reader, digest []byte, opts crypto.SignerOpts) ([]byte, error) {
	...
	if err := s.ctx.SignInit(s.session, []*pkcs11.Mechanism{mech}, s.keyHandle); err != nil {
		return nil, fmt.Errorf("hsm: C_SignInit failed: %v", err)
	}
	sig, err := s.ctx.Sign(s.session, signData)
	if err != nil {
		return nil, fmt.Errorf("hsm: C_Sign failed: %v", err)
	}
...
func (s *hsmSigner) prepareSigning(digest []byte, opts crypto.SignerOpts) (
	...
	default:
		return nil, nil, fmt.Errorf("hsm: unsupported key type %T", s.publicKey)
	}
}
