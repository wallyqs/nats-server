...
func (js *jetStream) monitorCluster() {
	...
	doSnapshot := func(force bool) {
		...
		if !fallbackSnapshot {
			...
			if nsz >= 10*szthresh {
				s.rateLimitFormatWarnf("JetStream cluster metalayer log size has exceeded async threshold, will fall back to blocking snapshot")
				fallbackSnapshot = true
			}
		}
		...
		if fallbackSnapshot {
			...
			if err != nil {
				...
				s.rateLimitFormatWarnf("Metalayer blocking snapshot took %.3fs (streams: %d, consumers: %d, compacted: %s)",
					took.Seconds(), nsa, nca, bytesN(csz, 3))
...
func (js *jetStream) encodeMetaSnapshot(streams map[string]map[string]*streamAssignment, bufs *s2Buffers) ([]byte, int, int, error) {
	...
	// Pre-allocate flat slices for all consumers and ClientInfo structs
	// to avoid per-element heap allocations (150K+ at scale).
	allConsumers := make([]writeableConsumerAssignment, 0, nca)
	allClients := make([]ClientInfo, 0, nsa+nca)
		...
		for _, sa := range asa {
			...
			for _, ca := range sa.consumers {
				allClients = append(allClients, ClientInfo{
					...
					Service: ca.Client.Service,
					Cluster: ca.Client.Cluster,
				})
				allConsumers = append(allConsumers, writeableConsumerAssignment{
					Client:     &allClients[len(allClients)-1],
					Created:    ca.Created,
					Name:       ca.Name,
					Stream:     ca.Stream,
			...
			allClients = append(allClients, ClientInfo{
				...
				Service: sa.Client.Service,
				Cluster: sa.Client.Cluster,
			})
	...
	if took > 2*time.Second {
		js.srv.rateLimitFormatWarnf("Metalayer snapshot generation took %.3fs (streams: %d, consumers: %d, marshal: %.3fs, s2: %.3fs, uncompressed: %s, compressed: %s)",
			took.Seconds(), nsa, nca, mend.Seconds(), cend.Seconds(), bytesN(uint64(len(b)), 3), bytesN(uint64(len(snap)), 3))
	}
